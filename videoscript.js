// GLOBAL VARIABLE
const API_KEY1 = "AIzaSyAlEvMbmftB0rXbI6FHhF78Z3-VwHJh-mU";
let selectedVideo;
let videoIdStore;
let commentObj;
...
const mainData1 = {
    ...
    "snippet": {
        ...
        "title": "I Am Using Hack In Free Fire ? 😨 Funny Gloo Wall Challenge In Free Fire",
        "description": "I Am Using Hack In Free Fire ? Funny Gloo Wall Challenge In Free Fire FOLLOW ME ON INSTAGRAM ...",
        "thumbnails": {
            "default": {
    ...
    targetele.innerHTML=`
         ...
         <div class="playback-body">
          ...
          <div class="playbackB1">
             <img src=${selectedVideo.channelDetails.snippet.thumbnails.high.url} alt="channel loading">
             
             <div class="channelname">
                 <h4>${selectedVideo.snippet.channelTitle}</h4>
                 <p>${getsubscriber(selectedVideo.channelDetails.statistics.subscriberCount)}</p>
             </div> 
         ...
         <div class="playbackB2">
             <div class="like-dislike">
                 ...
                 <p class="likecount">${getlikecount(selectedVideo.videoDetails.likeCount)}</p>
     ...
     <div>
        <div class="video-description">
          <p> ${getViewnos(countViews)} , ${getTimeDifference(selectedVideo.snippet.publishTime)}</p>
...
async function fetchRelVideo(key,vos){
    try{
        const response = await fetch(`${BASE_URL}/search?key=${API_KEY}&q=${key}&maxResults=${vos}&part=snippet`);
...
function relatablevideos(data){
    ...
    data.forEach((videosRl) => {
        ...
        divt.innerHTML=`
           <div class="rvideo-1">
                <img src= ${videosRl.snippet.thumbnails.high.url} alt="please wait">
            </div>

           <div class="rvideo-2">
              <h4>${videosRl.snippet.title}</h4>
              <p> ${videosRl.snippet.channelTitle}</p>
              <p>${getViewnos(countCheck)} &bull;  ${getTimeDifference(videosRl.snippet.publishTime)}  </p>
            </div>
        `
