const API_KEY = "AIzaSyAlEvMbmftB0rXbI6FHhF78Z3-VwHJh-mU";
...
const mainData = {
    ...
    "snippet": {
        ...
        "description": "Top 10 Highest Grossing Movies In The World #shorts #movies #viralshorts #theavengers #thelionking #jurrassicworld ...",
        "thumbnails": {
            "default": {
...
async function fetchVideo(searchTerm,videoNos){
    try{
        const response = await fetch(`${BASE_URL}/search?key=${API_KEY}&q=${searchTerm}&maxResults=${videoNos}&part=snippet`);
...
// fetchVideo("random", 25)

// Adding Video object to render Array
async function videoViews(id){

    try{
        const response = await fetch(`${BASE_URL}/videos?key=${API_KEY}&part=statistics&id=${id}`);
        const res = await response.json();
...
async function channelInfo(id){
    try{
    const response = await fetch(`${BASE_URL}/channels?key=${API_KEY}&part=statistics&part=snippet&id=${id}`);
    const res =  await response.json();
    return (res.items[0]);
    }
    catch(err){
...
function renderVideos(){
    ...
    realHomeData.forEach((videoR)=>{
        ...
        divt.innerHTML=`
        <div class="videoImg">
           <img  src=${videoR.snippet.thumbnails.medium.url} alt="thumbnail loading">
        </div>
        <div class="videobody">
         ...
         <div class="videoFooter">
          ...
          <p>${getViewnos(countCheck)} ${getTimeDifference(videoR.snippet.publishTime)}</p>
